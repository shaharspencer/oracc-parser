...
def load_word_csvs_from_zenodo(
    zenodo_url: str,
    project: str,
) -> dict[str, pd.DataFrame]:
    """Download a project's word CSV ZIP from Zenodo into memory.

    No files are written to disk. The ZIP is streamed into a BytesIO buffer
    and each inner CSV is read directly into a DataFrame.

    The expected filename in the Zenodo record is ``{project_slug}_words.zip``
    (e.g. ``saao-saa01_words.zip``), containing one ``{text_id}.csv`` per text.

    Args:
        zenodo_url: Zenodo record URL (e.g. ``"https://zenodo.org/records/12345"``).
        project: ORACC project path (e.g. ``"saao/saa01"``).
    ...
    from oracc_parser.download.fetch_data import get_zenodo_files

    project_slug = project.replace("/", "-")
    target_filename = f"{project_slug}_words.zip"

    files = get_zenodo_files(zenodo_url)
    match = next((f for f in files if f["filename"] == target_filename), None)
    if match is None:
        available = [f["filename"] for f in files]
        raise FileNotFoundError(
            f"No word CSV archive for '{project}' found in Zenodo record. "
            f"Expected '{target_filename}'. Available: {available}"
        )

    logger.info(
        f"Downloading {target_filename} from Zenodo "
        f"({match['size'] / 1e6:.1f} MB)..."
    )
    resp = requests.get(match["url"], stream=True, timeout=60)
    resp.raise_for_status()
    total = int(resp.headers.get("content-length", 0))

    buffer = io.BytesIO()
    with tqdm(total=total, unit="B", unit_scale=True, desc=target_filename) as bar:
    ...
    with zipfile.ZipFile(buffer, "r") as z:
        for name in z.namelist():
            if not name.endswith(".csv"):
                continue
            text_id = Path(name).stem
            with z.open(name) as f:
                result[text_id] = pd.read_csv(
                    io.TextIOWrapper(f, encoding="utf-8"),
                    dtype=str,
                    keep_default_na=False,
                )

    logger.info(f"Loaded {len(result)} word CSV(s) for '{project}' from Zenodo")
    return result
