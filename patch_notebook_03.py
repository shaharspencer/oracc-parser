...
for cell in cells:
    ...
    if "## 2. POS masking reference" in src:
        ...
        demo_code = {
            ...
            "outputs": [],
            "source": [
                "from oracc_parser import parse_project_from_oracc, RunConfig, get_transliterations, get_unicode_texts\n",
                "\n",
                "PROJECT = \"saao/saa01\"\n",
                "\n",
                "# Strict word-level filtering: words with >30% broken signs \u2192 replaced with X\n",
                "rec_strict = parse_project_from_oracc(PROJECT, config=RunConfig(limit=2, max_break_fraction=0.3))\n",
                "\n",
                "# Liberal (default): keep all words regardless of damage\n",
                "rec_liberal = parse_project_from_oracc(PROJECT, config=RunConfig(limit=2, max_break_fraction=1.0))\n",
                "\n",
